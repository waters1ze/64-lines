...
export async function sendCourseDeliveryEmail(to: string, courseName: string, fileUrl: string) {
  const html = `
    <div style="font-family: sans-serif; max-w-xl; margin: auto;">
      ...
      <div style="margin: 30px 0;">
        <p style="margin-bottom: 10px;">Вы также можете открыть материалы по прямой ссылке:</p>
        <a href="${fileUrl}" style="display: inline-block; padding: 10px 20px; background-color: #000; color: #fff; text-decoration: none; border-radius: 6px; font-weight: bold;">
          Открыть курс
        </a>
      </div>
